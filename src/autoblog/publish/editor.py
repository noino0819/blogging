    ...
    def publish_inplace(
        ...
        def _anchor_segment(seg_idx):
            ...
            if seg_idx == 0 or n_phys_video == 0:
                # 첫 미디어가 보존된 외부 이미지(협찬 배너)면 반드시 그 '뒤'에 쌓는다 — 배너
                # 최상단은 불변식(체험단 규정: 고지는 글 상단). 폴백(첫 미디어 앞)으로 넣으면
                # 본문이 배너 위로 올라가 배너가 밀리고, 블록별로 성공/실패가 섞이면 배너를
                # 사이에 두고 찢어지기까지 한다(2026-08-13 실사고 — 스티커가 배너 위로 밀림).
                # 앵커를 끝내 못 잡으면 잘못된 배치로 저장하느니 저장 전 중단(멱등) →
                # 실패 탭에서 수동 재시도.
                if ext["lead"]:
                    if self._anchor_after_photo(0):
                        return
                    raise PublishAborted(
                        "협찬 배너 뒤 삽입 위치를 잡지 못했어요 — 배너가 최상단에서 밀린 채 "
