...
function renderPmeta(){
  ...
  const tg=$('#thumbgen'); if(tg) tg.onclick=runThumbGen;
}
// ===== 멀티 탭(워크스페이스) =====
...
async function runThumbGen(){
  ...
  const kw=kwGet();
  const title=prompt('썸네일 중앙에 크게 들어갈 이름 (가게/제품명)', kw[0]||'');
  ...
  try{
    const r=await fetch('/api/thumbnail',{method:'POST',headers:{'content-type':'application/json'},
      body:JSON.stringify({photo:src,title:(title||'').trim()})});
    const d=await r.json();
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _gen_thumbnail(self, body):
            ...
            if is_video(photo.name):
                ...
                return
            try:
                png = generate_thumbnail(str(photo), (body.get("title") or "").strip())
