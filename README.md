...
src/autoblog/
  ...
  vision.py            # Vision LLM 연동 (Ollama, 이미지 스펙/사진 분류)
  llm.py               # Ollama 텍스트 LLM 공통
  draft/               # 2단계 · 초안 작성
    prompts.py         #   베이스 프롬프트 로딩(config/prompts/default.md, 편집 가능)
    prompt.py          #   프롬프트 계층 조립 + 사실카드 렌더링
    rules.py           #   공통 규칙 토글(선택적 추가 규칙)
    style.py           #   문체 프로파일(과거글 학습 + 톤 지시)
...
uv run autoblog draft "경험 메모" --place-url "<플레이스 URL>"  # 경험 중심 초안 생성
