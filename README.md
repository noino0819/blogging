...
**사진과 가게·상품 정보만 넣으면, 사용자 경험을 중심으로 한 네이버 블로그 글을 자동으로 써서 게시까지 해주는 데스크톱 프로그램.**

수집 → 초안 → 서식 → 네이버 게시를 한 흐름으로 잇는다. 글쓰기·수집·초안 생성은 **클라우드 LLM API**(Claude / GPT / Gemini)로, 네이버 게시는 **내 PC의 실제 브라우저**로 돈다.
...
입력(사진 + 경험 메모 + 가게/상품 URL + 선택: 톤·문체·가이드라인)
  │
  ├─ 1단계  정보 수집        네이버 검색 API + 플레이스/쇼핑 스크래핑 + 사진 자동분류(Vision)
  │                          → 사실 카드(JSON)
  ├─ 2단계  초안 작성        텍스트 LLM(Claude/GPT/Gemini)이 경험 중심 글 생성
  │                          → 강조·구분선·인용구·스티커·사진 배치 마커 자동 삽입
  ├─ 3단계  포맷 & 스타일    모바일 최적화 후처리(줄바꿈 균형, ! → .ᐟ 등)
  └─ 4단계  네이버 게시      Smart Editor 자동화(Playwright) → 임시저장 / 발행
```
...
| **프롬프트** | 기본 글쓰기 프롬프트 직접 편집·저장 |
| **문체** | 과거 글을 학습한 문체 프로파일 관리 |
| **모델** | 텍스트/비전 모델 선택(프리셋) + API 키 상태 |
| **설정** | 글쓰기 규칙 토글 · API 키 입력 |
...
```bash
# 네이버 지역검색 API — 가게 식별·주소·좌표 수집 (developers.naver.com, 사실상 무료)
NAVER_CLIENT_ID=...
NAVER_CLIENT_SECRET=...
NAVER_BLOG_ID=...            # 게시 대상 블로그 ID (첫 실행 시 받아 저장돼도 됨)

# 텍스트/비전 LLM — 쓰는 것만 채우면 됨 (models 탭에서 모델 선택)
...
```bash
uv run autoblog ui           # 유저 화면(로컬 웹) — 이게 메인 진입점
...
모델명은 코드에 박지 않고 [config/models.yaml](config/models.yaml)에서 읽는다. 프리셋(claude / claude_sonnet / gpt / gemini)을 고르거나 텍스트·비전을 독립 선택한다. 비전(사진 분류·상품 상세 전사)은 저렴한 Gemini Flash를 기본으로 쓴다.
...
src/autoblog/
  ...
  collect/               # 1단계 · 정보 수집
    ...
    place.py, place_detail.py   #   맛집: 검색 API + 플레이스 상세(apollo state)
